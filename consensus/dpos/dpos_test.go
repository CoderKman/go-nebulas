...
func TestConcurrentPacking(t *testing.T) {
	...
	tx := core.NewTransaction(dpos.chain.ChainID(), coinbase, coinbase, util.NewUint128FromInt(1), 1, core.TxPayloadBinaryType, []byte{}, core.TransactionGasPrice, util.NewUint128FromInt(200000))
	manager.SignTransaction(coinbase, tx)
	dpos.chain.TransactionPool().Push(tx)
